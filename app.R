...
ui <- fluidPage( #positioned as top to bottom as it appears on website
  ...
  img(width = "50%", src='bcl.jpg', align = "center"), ############### <- FEATURE 1: ADD AN IMAGE (must also add image into a folder called www)
  ...
  ) ############### <- FEATURE 2: ADD COUNTRY FILTER with DROP-DOWN options allowing for functionality such as: SELECT ALL, ADDING MUTLIPLE COUNTRIES, DESELECT etc. To use this functionality must use pickerInput() instead of selectInput()
,
...
#observe(cat(input$priceInput)) #useful for debugging
  ...
  output$alcohol_hist <- #output element for the histogram plot portion of this shiny app
    renderPlot({
      filtered_data() %>%ggplot(aes(Alcohol_Content)) + geom_histogram(color="grey60", fill= "cornsilk") +
        geom_density(color=NA, fill="blue", alpha=0.2)+ geom_line(stat='density', color="red") ############### <- FEATURE 3: adding a DENSITY DISTRIBUTION layer to the histogram & colors to embellish
